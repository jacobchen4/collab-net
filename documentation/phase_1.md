...
DBLP's existing records provide a disambiguation suffix for  athors with a repeated name (e.g. David Lo 0001). This system, however, does not cover all possible cases of ambiguous authorship. We chose to further disambiguate this data as mentioned in the paragraph above; this method, however, led to certain authors being joined erroneously. For example, authors Yan Liu and Yang Liu were joined under the reasoning that Yang was a misspelling of Yan. To remedy this, we manually reviewed author pairs which would have been merged by our original heuristic (documented under /disambiguation/).
...
[DBLP XML Index](https://dblp.org/xml/): directs you to the DBLP XML index, where you can find the GZIP dump file with its corresponding DTD.
